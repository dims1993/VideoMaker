...
export function TextArea(props: React.TextareaHTMLAttributes<HTMLTextAreaElement>) {
  ...
  return (
    <textarea
      {...rest}
      className={`mt-1 min-h-[88px] w-full rounded-xl border border-slate-200 bg-slate-50/80 px-3 py-2 text-sm text-slate-900 outline-none ring-emerald-500/30 focus:border-emerald-500 focus:bg-white focus:ring-2 ${className}`}
    />
  );
}
