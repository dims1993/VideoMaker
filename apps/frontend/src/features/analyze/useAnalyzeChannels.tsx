...
export function useAnalyzeChannels(opts: {
  workApplied: string;
  ...
  const [channelSort, setChannelSort] = useState<
    "subs" | "views" | "videos" | "views_per_video" | "views_per_sub"
  >("subs");
  ...
  const [selectedChannel, setSelectedChannel] =
    useState<ChannelSearchItem | null>(null);

  const [savedChannels, setSavedChannels] = useState<SavedChannelItem[]>([]);

  // ✅ PRIMERO EL STATE
  const [selectedSavedChannelId, setSelectedSavedChannelId] = useState<
    string | null
  >(null);

  // ✅ DESPUÉS LA FUNCIÓN
  const setSelectedSavedChannelIdPersisted = useCallback(
    (id: string | null) => {
      setSelectedSavedChannelId(id);

      if (id) {
        localStorage.setItem("lastSelectedChannelId", id);
      } else {
  ...
  const [analyzePanel, setAnalyzePanel] = useState<"search" | "saved">(
    "search",
  );

  const [savedChannelVideos, setSavedChannelVideos] = useState<
    ChannelVideoItem[]
  >([]);
  ...
  const fetchSession = useCallback(async (): Promise<Session | null> => {
    try {
      const r = await fetch(
        `/api/session?work=${encodeURIComponent(workApplied)}`,
      );
  ...
  const waitForChannelJob = useCallback(
    async (
      step: "channel_sync" | "channel_backfill",
      channelId: string,
  ...
  const refreshSavedChannelVideos = useCallback(async (channelId: string) => {
    try {
      const r = await fetch(
        `/api/channels/${encodeURIComponent(channelId)}?videos_limit=100`,
      );
  ...
  // ✅ MOVIDO DESPUÉS DE refreshSavedChannelVideos
  useEffect(() => {
    const last = localStorage.getItem("lastSelectedChannelId");

    if (last) {
      setSelectedSavedChannelId(last);
      refreshSavedChannelVideos(last);
    }
  ...
  const selectedSavedChannel = useMemo(
    () =>
      selectedSavedChannelId
        ? (savedChannels.find((x) => x.channel_id === selectedSavedChannelId) ??
          null)
        : null,
  ...
  const activeThresholdProfile = useMemo(
    () => profileForCategory(selectedSavedChannel?.internal_category),
    [selectedSavedChannel?.internal_category],
  );

  const graders = useMemo(
    () => createVideoMetricGraders(activeThresholdProfile),
  ...
  const savedById = useMemo(() => {
    ...
    for (const c of savedChannels) {
      m.set(c.channel_id, c);
    }
  ...
  useEffect(() => {
    ...
    setEditLang(
      ((selectedSavedChannel.language as "" | "es" | "en") || "") as
        | ""
        | "es"
        | "en",
    );
  ...
  const selectedChannelSyncState = useMemo(() => {
    ...
    if (st.step === "channel_sync" || st.step === "channel_scan") {
      return st;
    }
  ...
  const allVisibleVideoIds = useMemo(
    () => savedChannelVideos.slice(0, 100).map((v) => v.video_id),
    [savedChannelVideos],
  );
  ...
  const allSelected = useMemo(
    () =>
      ...
      allVisibleVideoIds.every((id) => selectedVideoIds.includes(id)),
    [allVisibleVideoIds, selectedVideoIds],
  );

  const metricPillFn = useCallback(
    (value: string, g: MetricGrade) => <MetricPill value={value} grade={g} />,
    [],
  );
  ...
  return {
    ...
    selectedSavedChannelId,
    setSelectedSavedChannelId: setSelectedSavedChannelIdPersisted,
