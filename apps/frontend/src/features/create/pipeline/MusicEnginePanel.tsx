...
export function MusicEnginePanel({
  run,
  workApplied,
  musicStepState,
  refreshPipeline,
  ...
  workApplied: string;
  musicStepState: string;
  refreshPipeline?: () => void | Promise<void>;
}) {
  const [artifact, setArtifact] = useState<ArtifactResp | null>(null);
  const [loading, setLoading] = useState(false);

  const canRun = musicStepState !== "running";

  const load = async () => {
    setLoading(true);
    try {
      const r = await fetch(
        `/api/pipeline/music-plan?work=${encodeURIComponent(workApplied)}`,
      );
  ...
  const handleGenerate = () =>
    run("Music Engine · generar", async () => {
      const r = await fetch("/api/pipeline/step/rerun", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ work: workApplied, step_id: "music_engine" }),
      });
  ...
  return (
    <div className="space-y-4">
      <PipelineStepConfirmBar
        stepId="music_engine"
        stepLabel="Music Engine"
        workApplied={workApplied}
        stepState={musicStepState}
        run={run}
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
        <div>
          <div className="text-sm font-semibold text-slate-900">
            Music Engine (music_plan.json)
          </div>
          <div className="text-xs text-slate-500">
            Ancla beats a huecos y bloques del audio real (audio_timeline.json).
            Ejecuta después de Voiceovers.
          </div>
        </div>
        <Btn onClick={handleGenerate} disabled={!canRun}>
          {musicStepState === "running" ? "Generando…" : "Generar / Re-generar"}
        </Btn>
      </div>
