...
export function VoiceoverEnginePanel({
  run,
  workApplied,
  voiceoverEngineStepState,
}: {
  run: (label: string, fn: () => Promise<void>) => void;
  workApplied: string;
  voiceoverEngineStepState: string;
}) {
  const [artifact, setArtifact] = useState<ArtifactResp | null>(null);
  const [loading, setLoading] = useState(false);

  const canRun = voiceoverEngineStepState !== "running";

  const load = async () => {
    setLoading(true);
    try {
      const r = await fetch(
        `/api/pipeline/voiceover-plan?work=${encodeURIComponent(workApplied)}`,
      );
  ...
  }, [workApplied, voiceoverEngineStepState]);
  ...
  const handleGenerate = () =>
    run("Voiceover Engine · generar", async () => {
      const r = await fetch("/api/pipeline/step/rerun", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ work: workApplied, step_id: "voiceover_engine" }),
      });
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
        <div>
          <div className="text-sm font-semibold text-slate-900">
            Voiceover Engine (voiceover_plan.json)
          </div>
          <div className="text-xs text-slate-500">
            Genera un plan de voz y prepara `script_for_tts.txt` (pausas por
            párrafo).
          </div>
        </div>
        <Btn onClick={handleGenerate} disabled={!canRun}>
          {voiceoverEngineStepState === "running"
            ? "Generando…"
            : "Generar / Re-generar"}
        </Btn>
      </div>
