...
def _build_system_prompt(avatar_description: str, target_generator: str) -> str:
    ...
    gen_note = generator_notes.get(target_generator, generator_notes["midjourney"])
...
3. Define una situación visual breve en español (qué hace el avatar, qué hay de fondo)
4. Escribe el ai_prompt COMPLETO EN INGLÉS describiendo: avatar + expresión + situación + estilo
5. {gen_note}
...
def generate_avatar_image_prompts(
    ...
    *,
    ...
    system_prompt = _build_system_prompt(avatar_description, target_generator)
